...
echo º                         Sauvegarde Des EARs V1.6                            º
...
rem creation du repertoire de l'année
...
echo ÉÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ»
echo º            Sauvegarde De %EAR% / %IPEAR%        
echo ÈÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼

echo ÍÍÍÍÍÍÍÍÍ[ 01. Recherche d une precedente sauvegarde de %EAR%

IF exist %Racine%\%RepSave%\%year%\%year%-%month%\%udate%\%EAR%.rtf (
	echo ÍÍÍÍÍÍÍÍÍ[ 02. Sauvegarde detecte pour %EAR%
...
IF NOT exist %Racine%\%RepSave%\%year%\%year%-%month%\%udate%\%EAR%.rtf (
	echo ÍÍÍÍÍÍÍÍÍ[ 02. Aucune sauvegarde detecte pour %EAR%
	echo ÍÍÍÍÍÍÍÍÍ[ 03. Creation de la sauvegarde de %EAR%
	)

echo ÍÍÍÍÍÍÍÍÍ[ 04. Connexion a %EAR%

"%Racine%\%DossierScript%\plink.exe" -ssh %IDSave%@%IPEAR% -pw %MDPSAVE% "copy run tftp://%ServeurTFTP%/%EAR%.rtf"

echo.
echo ÍÍÍÍÍÍÍÍÍ[ 05. Deconnexion de %EAR%
echo ÍÍÍÍÍÍÍÍÍ[ 06. Deplacement du fichier de sauvegarde %EAR%.rtf

move %Racine%\%RepSave%\%EAR%.rtf %Racine%\%RepSave%\%year%\%year%-%month%\%udate%\%EAR%.rtf

echo ÍÍÍÍÍÍÍÍÍ[ 07. Fin sauvegarde de la configuration de %EAR%
echo ÍÍÍÍÍÍÍÍÍ[ 08. Verification de la sauvegarde de %EAR%

IF exist %Racine%\%RepSave%\%year%\%year%-%month%\%udate%\%EAR%.rtf (
	set /a Seffectue = Seffectue + 1
	)
IF NOT exist %Racine%\%RepSave%\%year%\%year%-%month%\%udate%\%EAR%.rtf (
	echo ÍÍÍÍÍÍÍÍÍ[ 09. Echec de la sauvegarde de %EAR%
...
echo.
echo ÍÍÍÍÍÍÍÍÍ[ 05. Deconnexion de %EAR%
echo ÍÍÍÍÍÍÍÍÍ[ 06. Deplacement du fichier de sauvegarde %EAR%.txt
...
echo ÍÍÍÍÍÍÍÍÍ[ 08. Verification de la sauvegarde de %EAR%
IF exist %Racine%\%RepSave%\%year%\%year%-%month%\%udate%\%EAR%.txt (
	echo ÍÍÍÍÍÍÍÍÍ[ 09. Reussite de la sauvegarde de %EAR%
	set /a Seffectue = Seffectue + 1
	)
IF NOT exist %Racine%\%RepSave%\%year%\%year%-%month%\%udate%\%EAR%.txt (
	echo ÍÍÍÍÍÍÍÍÍ[ 09. Echec de la sauvegarde de %EAR%
